package UI.abstract.component.control.mc
{
	...
	public class MovieClip extends Container implements IAnimatable
	{
		...
		public function MovieClip(str:String = null, fps:Number=12)
		{
			_bitmap = new Bitmap();
			addChild( _bitmap );
			_fps = fps;
		...
		public function set texture(imageResource:ImageResource):void{
			...
			if(imageResource && imageResource.frame){
				_bitmap.x = -imageResource.frame.x;
				_bitmap.y = -imageResource.frame.y;
			}else{
				_bitmap.x = 0;
				_bitmap.y = 0;
			}
